...
impl SessionStore {
    pub async fn new(database_url: &str) -> sqlx::Result<Self> {
        ...
        if let Some(rest) = database_url.strip_prefix("sqlite://") {
            if !rest.starts_with(':') { // avoid in-memory or special forms
                if let Some(parent) = Path::new(rest).parent() { if !parent.as_os_str().is_empty() { let _ = std::fs::create_dir_all(parent); } }
            }
        }
    ...
    async fn run_migrations(pool: &SqlitePool) -> sqlx::Result<()> {
        ...
        sqlx::query(r#"CREATE TABLE IF NOT EXISTS messages (
        ...
        sqlx::query(r#"CREATE INDEX IF NOT EXISTS idx_messages_session ON messages(session_id)"#).execute(pool).await?;
        sqlx::query(r#"CREATE INDEX IF NOT EXISTS idx_messages_session_role ON messages(session_id, role)"#).execute(pool).await?;
        sqlx::query(r#"CREATE INDEX IF NOT EXISTS idx_messages_features ON messages(features)"#).execute(pool).await?;
        Ok(())
    }

    /// Explicit helper to idempotently create / migrate the database without keeping a pool instance.
    pub async fn ensure_database(database_url: &str) -> sqlx::Result<()> { let store = Self::new(database_url).await?; let _ = store.pool.acquire().await?; Ok(()) }

    pub async fn create_session(&self, device_id: &str, port_name: Option<&str>) -> sqlx::Result<Session> {
        let now = Utc::now();
        ...
        Ok(Session { id, device_id: device_id.to_string(), port_name: port_name.map(|s| s.to_string()), created_at: now, updated_at: now, closed: 0 })
    }
    ...
    pub async fn append_message(&self, session_id: &str, role: &str, direction: Option<&str>, content: &str, features: Option<&str>, latency_ms: Option<i64>) -> sqlx::Result<(i64, DateTime<Utc>)> {
        let now = Utc::now();
        ...
        sqlx::query("UPDATE sessions SET updated_at = ?1 WHERE id = ?2")
            .bind(now).bind(session_id).execute(&mut *conn).await?;
        Ok((last_id, now))
    }

    pub async fn list_messages(&self, session_id: &str, limit: i64) -> sqlx::Result<Vec<Message>> {
        sqlx::query_as::<_, Message>("SELECT * FROM messages WHERE session_id = ?1 ORDER BY id ASC LIMIT ?2")
            .bind(session_id).bind(limit).fetch_all(&self.pool).await
    }

    pub async fn filter_messages(&self, session_id: &str, role: Option<&str>, feature_substring: Option<&str>, direction: Option<&str>, limit: i64) -> sqlx::Result<Vec<Message>> {
        ...
        if role.is_some() { sql.push_str(" AND role = ?"); }
        if direction.is_some() { sql.push_str(" AND direction = ?"); }
        if feature_substring.is_some() { sql.push_str(" AND features LIKE ?"); }
        sql.push_str(" ORDER BY id ASC LIMIT ?");

        let mut query = sqlx::query_as::<_, Message>(&sql).bind(session_id);
        if let Some(r) = role { query = query.bind(r); }
        if let Some(d) = direction { query = query.bind(d); }
        if let Some(f) = feature_substring { query = query.bind(format!("%{}%", f)); }
    ...
    pub async fn export_features_index(&self, session_id: &str) -> sqlx::Result<serde_json::Value> {
        ...
        let rows = sqlx::query("SELECT features FROM messages WHERE session_id = ?1 AND features IS NOT NULL")
    ...
    pub async fn export_messages_with_features(&self, session_id: &str, feature_filter: Option<&str>) -> sqlx::Result<serde_json::Value> {
        let mut q = "SELECT * FROM messages WHERE session_id = ?1".to_string();
        if feature_filter.is_some() { q.push_str(" AND features LIKE ?2"); }
        q.push_str(" ORDER BY id ASC");
        let mut query = sqlx::query_as::<_, Message>(&q).bind(session_id);
        if let Some(f) = feature_filter { query = query.bind(format!("%{}%", f)); }
    ...
    pub async fn close_session(&self, session_id: &str) -> sqlx::Result<()> {
        sqlx::query("UPDATE sessions SET closed = 1, updated_at = ?1 WHERE id = ?2")
            .bind(Utc::now()).bind(session_id).execute(&self.pool).await?;
        Ok(())
    }
...
fn ft_parse(s: &str) -> Result<DateTime<Utc>, chrono::ParseError> { s.parse::<DateTime<Utc>>() }
...
mod tests {
    ...
    fn memory_db() -> &'static str { "sqlite::memory:?cache=shared" }

    #[tokio::test]
    async fn create_and_get_session() {
        let store = SessionStore::new(memory_db()).await.expect("init store");
        let s = store.create_session("device-1", Some("COM1")).await.expect("create");
        let fetched = store.get_session(&s.id).await.expect("get").expect("exists");
    ...
    let (id1, ts1) = store.append_message(&s.id, "system", None, "init", None, None).await.expect("append1");
    let (id2, ts2) = store.append_message(&s.id, "user", Some("sent"), "command RUN", Some("command"), Some(12)).await.expect("append2");
    let (id3, ts3) = store.append_message(&s.id, "device", Some("received"), "ERR timeout", Some("error,warning"), Some(34)).await.expect("append3");
    assert!(id1 < id2 && id2 < id3, "message ids should be ascending");
    assert!(ts1 <= ts2 && ts2 <= ts3, "timestamps should be non-decreasing");

        let all = store.list_messages(&s.id, 100).await.expect("list");
        assert_eq!(all.len(), 3);

        // Filter by role
        let user_msgs = store.filter_messages(&s.id, Some("user"), None, None, 50).await.expect("filter role");
        assert_eq!(user_msgs.len(), 1);
        assert_eq!(user_msgs[0].role, "user");

        // Filter by direction
        let received = store.filter_messages(&s.id, None, None, Some("received"), 50).await.expect("filter direction");
        ...
        let error_like = store.filter_messages(&s.id, None, Some("error"), None, 50).await.expect("filter feature");
        assert_eq!(error_like.len(), 1);
        assert!(error_like[0].features.as_deref().unwrap().contains("error"));

        // Feature index
        let idx = store.export_features_index(&s.id).await.expect("feature index");
        let counts = idx.get("feature_counts").and_then(|v| v.as_object()).expect("counts object");
        assert_eq!(counts.get("command").and_then(|v| v.as_u64()).unwrap_or(0), 1);
        assert_eq!(counts.get("error").and_then(|v| v.as_u64()).unwrap_or(0), 1);
        assert_eq!(counts.get("warning").and_then(|v| v.as_u64()).unwrap_or(0), 1);
    }
}
