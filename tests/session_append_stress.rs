...
fn stress_append_ordering() {
    ...
    let th_out = thread::spawn(move || {
        ...
        loop {
            match out.read(&mut local) { Ok(0) => break, Ok(n) => { if let Ok(mut b) = out_clone.lock() { b.extend_from_slice(&local[..n]); } }, Err(_) => break }
        }
    });
    let th_err = thread::spawn(move || {
        let mut local = [0u8; 1024];
        loop { match err.read(&mut local) { Ok(0) => break, Ok(n) => { if let Ok(mut b) = err_clone.lock() { b.extend_from_slice(&local[..n]); } }, Err(_) => break } }
    });
    ...
    while Instant::now() < deadline { if has_response(&out_buf, 1) { break; } thread::sleep(Duration::from_millis(20)); }
    ...
    while Instant::now() < deadline_create { if has_response(&out_buf, 2) { break; } thread::sleep(Duration::from_millis(10)); }
    let session_id = extract_session_id(&out_buf).unwrap_or_else(|| {
        let out_snapshot = out_buf.lock().unwrap().clone();
        let err_snapshot = err_buf.lock().unwrap().clone();
        eprintln!("FAILED to extract session id. Raw STDOUT:\n{}", String::from_utf8_lossy(&out_snapshot));
        eprintln!("Raw STDERR:\n{}", String::from_utf8_lossy(&err_snapshot));
        panic!("session id");
    });
    ...
    while Instant::now() < deadline2 { if count_responses(&out_buf, 1000, 1000 + total as i64) >= total as usize { break; } thread::sleep(Duration::from_millis(25)); }
    assert!(count_responses(&out_buf, 1000, 1000 + total as i64) >= total as usize, "did not receive all append responses");
    ...
    while Instant::now() < deadline3 { if has_response(&out_buf, 5000) { break; } thread::sleep(Duration::from_millis(20)); }
    let msgs = extract_messages(&out_buf).expect("messages");
    assert!(msgs.len() as u32 >= total, "expected at least {total} messages, got {}", msgs.len());
    // Ascending by id
    let mut last_id = -1i64;
    for m in msgs.iter() { if let Some(id) = m.get("id").and_then(|v| v.as_i64()) { assert!(id > last_id, "message ids not strictly increasing: {} <= {}", id, last_id); last_id = id; } }
...
fn has_response(buf: &Arc<Mutex<Vec<u8>>>, id: i64) -> bool { parse_messages(&buf.lock().unwrap()).into_iter().any(|v| v.get("id").and_then(|i| i.as_i64()) == Some(id)) }
fn count_responses(buf: &Arc<Mutex<Vec<u8>>>, start: i64, end: i64) -> usize { parse_messages(&buf.lock().unwrap()).into_iter().filter(|v| { if let Some(id) = v.get("id").and_then(|i| i.as_i64()) { id >= start && id < end } else { false } }).count() }

fn extract_session_id(buf: &Arc<Mutex<Vec<u8>>>) -> Option<String> {
    for v in parse_messages(&buf.lock().unwrap()) { if v.get("id").and_then(|i| i.as_i64()) == Some(2) { if let Some(result) = v.get("result") { if let Some(export) = result.get("session") { if let Some(id) = export.get("id").and_then(|s| s.as_str()) { return Some(id.to_string()); } } if let Some(structured) = result.get("structuredContent") { if let Some(sess) = structured.get("session") { if let Some(id) = sess.get("id").and_then(|s| s.as_str()) { return Some(id.to_string()); } } } } } }
    None
}

fn extract_messages(buf: &Arc<Mutex<Vec<u8>>>) -> Option<Vec<Value>> {
    for v in parse_messages(&buf.lock().unwrap()) { if v.get("id").and_then(|i| i.as_i64()) == Some(5000) { if let Some(result) = v.get("result") { if let Some(structured) = result.get("structuredContent") { if let Some(msgs) = structured.get("messages").and_then(|m| m.as_array()) { return Some(msgs.clone()); } } } } }
    None
}
...
fn parse_messages(raw: &[u8]) -> Vec<Value> {
            ...
            if let Ok(s) = std::str::from_utf8(&raw[idx..]) {
                if let Some(term) = s.find("\r\n\r\n").or_else(|| s.find("\n\n")) {
                    ...
                    if let Some(len) = length {
                        ...
                        let end = start + len;
                        if end <= raw.len() {
                            if let Ok(val) = serde_json::from_slice(&raw[start..end]) { msgs.push(val); }
                            framed_cursor = end;
                            continue;
                        } else { break; }
                    }
    ...
    if let Ok(all) = std::str::from_utf8(raw) {
        for line in all.lines() {
            ...
            if t.starts_with('{') { if let Ok(val) = serde_json::from_str::<Value>(t) { msgs.push(val); } }
        }
    }
    msgs
}
